...
12. [How do we know the noise amplitude, and how do we know a threshold clears it?](#12-how-do-we-know-the-noise-amplitude-and-how-do-we-know-a-threshold-clears-it)
13. [If the sensor is mounted and an earthquake happens, does it still work?](#13-if-the-sensor-is-mounted-and-an-earthquake-happens-does-it-still-work)
...
## 12. How do we know the noise amplitude, and how do we know a threshold clears it?

You measure it, on the real hardware, over the interval the decision spans. Never
from a datasheet — that describes the part, not the installation.

### "The noise" is not one number

`accel_z` at rest, measured on this appliance:

| Sensor | within-minute | minute-to-minute | 2-hour span |
|---|---|---|---|
| SENSOR-001 | **0.000000** | **0.000000** | **0.000000** |
| SENSOR-002 | 0.000036 | 0.000122 | 0.000488 |
| SENSOR-003 | 0.000015 | 0.000051 | 0.000366 |
| SENSOR-004 | 0.000583 | 0.000109 | 0.000732 |

Three different noises, because a threshold decision spans a timescale. A check
comparing consecutive samples cares about the first column; one comparing today
against a baseline captured at commissioning cares about something wider than
any of them.

And SENSOR-001 reads **zero on all three** — not because it is quiet, but because
its output is quantised to one code in two hours. **A measured noise of zero
means the instrument cannot see its own noise**, and a margin computed from it is
infinite and meaningless.

### Noise grows with the window, which means you cannot extrapolate

The temperature offset between two sensors:

| Window | sd | range |
|---|---|---|
| 1 h | **0.020** | 0.482 – 0.576 |
| 6 h | 0.048 | 0.482 – 0.825 |
| 24 h | 0.058 | 0.409 – 0.825 |
| 72 h | **0.068** | 0.243 – 0.825 |

**3.4× worse from one hour to seventy-two.** That is drift, not white noise:
averaging does not help, and a threshold justified by a short measurement is not
justified at all.

### The trap: which noise

The identity check compared sensors by resting acceleration. 0.0133 g apart,
twenty-six times the 0.0005 g quantisation step — ample. It shipped. Then a
physical swap detected nothing:

```
difference between units      0.0133 g    <- the signal
shift caused by handling      0.0380 g    <- the noise that mattered
```

Picking a unit up moves it 0.038 g, cable strain flexing the package. Three times
the thing being measured.

> **You cannot identify a thing by a property that changes more when you touch it
> than it differs between things.**

The relevant noise is **the largest perturbation between the baseline and the
decision** — not the instrument's resolution. And for an identity check the
disturbance you must beat is the one that *accompanies* the event you are
detecting, because the event is somebody handling the sensor.

The tests passed on synthetic magnitudes with no handling term. **They tested the
arithmetic, not the physics.**

### How the appliance does this, concretely

| Mechanism | What it does |
|---|---|
| `alarms:vibration-survey` | 1.8 M real samples, median / p95 / p99.9 / peak |
| `sensor:noise-floor` | noise per channel at 1 h / 6 h / 24 h, and the margin every threshold has against it |
| quiet-minute filtering | measures the fingerprint only from minutes below 0.02 g |
| `tilt:baseline capture` | refuses outright if the window was disturbed above 0.05 g |
| `probe-wit-standard --verify` | records a channel responding to physical stimulus, and writes the transcript |
| `alarms:confirm-thresholds` | records the named person who checked a limit against a real source |

`sensor:noise-floor` on a live channel:

```
vib_velocity_x
  sensor         distinct         1h         6h        24h      unit
  SENSOR-001           11   0.015672   0.006429   0.009299     mm/s
      warning   vibration-velocity-SENSOR-001 = 3        191.4x the noise
      critical  vibration-velocity-SENSOR-001 = 10       638.1x the noise
  SENSOR-003           24   0.025410   0.011055   0.270782     mm/s
      warning   vibration-velocity-SENSOR-003 = 3         11.1x the noise
```

It refuses to report noise for a channel with too few distinct values, flags any
margin under 3×, and exits non-zero so a build can fail on it.

Note the evidence of the principle sitting in the survey's own output:
`vib_velocity_x` peaked at **476 mm/s** against a 10 mm/s critical limit. No silo
did that — somebody picked the sensor up. Set a threshold from `peak` and you get
a number no earthquake could reach.

### The rule

```
signal   = separation between the things you must tell apart
noise    = largest perturbation between baseline and decision   <- not the resolution
margin   = signal / noise

margin > ~10   usable
margin 3-10    usable with a refusal floor and a stated limit
margin < 3     do not ship it - find another property
```

**A property that is stable is worth more than one that is precise.** Acceleration
resolves 26× better than temperature separates, and temperature won, because it
barely moves when a human is in the room.

---

## 13. If the sensor is mounted and an earthquake happens, does it still work?

Yes — but "it" is three different jobs, and they behave differently.

| Job | Quantity | During shaking |
|---|---|---|
| **Which sensor is which** | chip temperature | property unaffected; the *check* pauses — see below |
| **What is the structure doing** | acceleration amplitude, velocity | **works — this is the job** |
| **Has it permanently moved** | tilt, from gravity | correctly declares itself untrustworthy |

### The vibration channels do their job

```
acceleration_amplitude    warn 0.5 g     crit 2 g
vibration_velocity        warn 3 mm/s    crit 10 mm/s   (ISO 10816)
```

### Tilt correctly suspends itself, and that is physics

```python
QUASI_STATIC_BAND = (0.85, 1.15)
# Gravity is 1 g; anything outside this band is being accelerated,
# and the angles are not orientation.
```

Tilt is computed from the *direction* of gravity. While the building swings, the
accelerometer measures gravity **plus** the building's motion, and that sum does
not point at the centre of the Earth. **No inclinometer can measure lean during
shaking.** An appliance reporting a confident tilt mid-earthquake would be lying.

So settlement is a **before/after** measurement:

```
before   tilt 0.66 deg    baseline
during   vibration alarms fire; tilt flagged implausible
after    tilt 0.94 deg    <- the settlement, measured once it is still again
```

### The identity check: the property survives, the implementation pauses

Temperature is indifferent to shaking — a die does not warm because a building
moves. So in principle the fingerprint works throughout.

**But `verifyWiring()` calls `measure()`, which filters to minutes quieter than
0.02 g.** During an earthquake there are no quiet minutes, so it returns null and
the check declines to answer. Not because temperature became unreliable, but
because the *gate* is on acceleration.

Practical effect: the check resumes within about ten minutes of the shaking
stopping, which for a settlement question is immaterial. The real limitation is a
site with **continuous** vibration — plant machinery, heavy traffic — where quiet
minutes may never arrive and the identity check would never run at all. That is
recorded here rather than fixed, because whether it matters depends on the site.

### The one timing caveat

The tilt alarm needs the deviation sustained for **3600 seconds**. So after an
earthquake that permanently leans the silo: vibration alarms fire immediately,
the new lean is visible on the dashboard as soon as shaking stops, and the
settlement *alarm* confirms about an hour later. Deliberate for a permanent slow
quantity — worth revisiting if immediate post-event assessment is wanted.
