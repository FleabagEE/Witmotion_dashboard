...
## ADR-026 — Read-only device paths hold an object that cannot write

**Decision.** Modules that access a sensor without writing to it receive a
`ReadOnlyModbus`, which exposes `read_holding_registers` and port lifecycle and
nothing else. They do not import `pymodbus`. `tests/test_readonly.py` parses each
such module and fails the build if it imports pymodbus or names a write.

**Context.** `discovery.py` claimed writes were "enforced structurally rather
than by convention" while importing `ModbusSerialClient` directly — the same
class `address_cli.py` uses to rewrite a sensor's address register and commit it
to flash. It held a fully write-capable object and promised not to call one of
its methods. No test checked the promise.

The failure mode needs no villain: a developer adds retry logic to discovery,
copies a block from `address_cli.py` to see how retries are done there, and one
line of it writes. Tests pass, because nothing tested this. Review shows three
lines of retry logic above a docstring saying writes are impossible. Weeks later
`qv-discover` — documented as safe on a live bus — rewrites a mounted sensor's
address mid-scan and takes it off the bus.

A write is now an `AttributeError` at first run rather than a policy violation
discovered in the field. The capability is removed, not forbidden.

**Cost.** The wrapper must be widened whenever a read-only path legitimately
needs a new read. That friction is the mechanism, not a side effect: widening it
is a visible diff against a docstring that argues the other way.

**What this is not.** It does not stop a determined contributor; four seconds of
typing restores the import, and `_client` is reachable. It is not meant to. It
raises the cost of an *accident* to near-infinity and the cost of an *intention*
to near-zero — the same trade as Rust's `unsafe` being a keyword rather than a
prohibition, `__user` annotations under sparse in the Linux kernel, and MISRA
deviations requiring written justification. Hostile contributors are a
merge-rights problem, not a type-system one.

**The general rule.** A safety claim that is not executable is worse than no
claim, because it stops a reviewer looking. Any comment in this repository
asserting that something *cannot* happen should be treated as a missing test.
